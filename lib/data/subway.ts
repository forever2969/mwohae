...
export const SUBWAY_LINES: SubwayLine[] = [
  ...
  },
  {
    ...
    stations: [
      ...
      '대공원', '과천', '정부과천청사', '인덕원', '평촌', '범계', '금정', '산본', '수리산',
      '대야미', '반월', '상록수', '한대앞', '중앙', '고잔', '초지', '안산', '신길온천',
      '정왕', '오이도',
    ],
  },
  {
    ...
    stations: [
      ...
      '신정', '목동', '오목교', '양평', '영등포구청', '영등포시장', '신길', '여의도', '여의나루',
      '마포', '공덕', '애오개', '충정로', '서대문', '광화문', '종로3가', '을지로4가', '동대문역사문화공원',
      '청구', '신금호', '행당', '왕십리', '마장', '답십리', '장한평', '군자', '아차산',
      '광나루', '천호', '강동', '길동', '굽은다리', '명일', '고덕', '상일동',
    ],
  },
  {
    line: 6,
  ...
  },
  {
    ...
    stations: [
      ...
      '까치울', '부천종합운동장', '춘의', '부천시청', '상동', '삼산체육관', '굴포천', '부평구청',
    ],
  },
  {
    line: 8,
    ...
    color: '#E6186C',
    stations: [
      '암사', '천호', '강동구청', '몽촌토성', '잠실', '석촌', '송파', '가락시장', '문정',
      '장지', '복정', '산성', '남한산성입구', '단대오거리', '신흥', '수진', '모란',
    ],
  },
  {
    line: 9,
...
export function getLine(line: number): SubwayLine | undefined {
  return SUBWAY_LINES.find((l) => l.line === line)
}

export function getStation(line: number, index: number, direction: 'left' | 'right'): string | undefined {
  const lineData = getLine(line)
  if (!lineData) return undefined
  const stations = direction === 'right' ? [...lineData.stations].reverse() : lineData.stations
  return stations[index]
}

export function getStationCount(line: number): number {
  return getLine(line)?.stations.length ?? 0
}
